...
| No note content, prompt text or response text reaches a log, a database row or an error string shown to the user. | `src-tauri/src/commands/ai.rs:21-22` for the rewrite path; `sanitize_update_error`, `src-tauri/src/commands/update.rs:293`, for the updater; `writ_core::activity::ActivityRecord` (`crates/writ-core/src/activity.rs:88`) has no content field, asserted over the serialised keys by `a_record_carries_no_key_that_could_hold_note_text` and again on a written line by `a_record_the_gate_writes_holds_no_note_text` | Shipped |
...
| A client that has not been approved is refused, and its call performs no read and no write. | `writ_mcp::consent::ConfigGate` over `[mcp] approved_clients`, `crates/writ-mcp/src/consent.rs:117`, wired at `crates/writ-cli/src/main.rs:194`; the check is the first line of every tool method (`ToolHost::allow`, `crates/writ-mcp/src/tools.rs:473`), so a refusal opens no file; `DenyAll` is still the default a host takes | Shipped |
...
| Consent is revocable in the app and by editing `config.toml`, and takes effect on the next call without a restart. | `ai.consented_hosts` (`crates/writ-core/src/config/ai.rs:56`) with `ai_consent_host` (`src-tauri/src/commands/ai.rs:355`) for hosts; `mcp_set_client_permission` and `mcp_forget_client` (`src-tauri/src/commands/activity.rs:146,159`) for clients, against the mtime-guarded `ConfigGate`; asserted by `an_approval_written_between_two_calls_lands_on_the_second`, `forgetting_a_client_returns_it_to_pending_on_the_next_call` and `an_unchanged_config_is_read_once_however_many_calls_arrive` | Shipped |
