      ...
      for (const tour of [true, false]) {
        for (const engaged of [true, false]) {
          for (const visible of [true, false]) {
            expect(shouldPostScene({ live, tour, engaged, visible }), JSON.stringify({ live, tour, engaged, visible })).toBe(
              live && tour && !engaged && visible,
            );
          }
        }
      }
...
describe('activeScene', () => {
  ...
  it('is hero above the first step', () => {
    expect(activeScene(LINE, steps(LINE + 1))).toBe('hero');
    expect(activeScene(LINE, [])).toBe('hero');
  });

  it('is the last step whose top has reached the line', () => {
    expect(activeScene(LINE, steps(LINE))).toBe('any-file');
    expect(activeScene(LINE, steps(LINE - 1700))).toBe('search');
  });

  it('keeps versions once the line is past the stage', () => {
    expect(activeScene(LINE, steps(-100000))).toBe('versions');
  });

  it('trusts the step the observer reports at the line over a rect a fraction of a pixel off it', () => {
    const boxes = steps(LINE + 0.77).map((box) => ({ ...box, atLine: box.name === 'any-file' }));
    expect(activeScene(LINE, boxes)).toBe('any-file');
  });

  it('takes the later of two steps that both touch the line', () => {
    const boxes = steps(LINE - 800).map((box) => ({ ...box, atLine: box.name === 'any-file' || box.name === 'markdown' }));
    expect(activeScene(LINE, boxes)).toBe('markdown');
  });
});
...
describe('createSceneDriver', () => {
  ...
  it('posts nothing before the app is ready, then the active scene when it is', () => {
    const driver = createSceneDriver({ tour: true, visible: true });
    const { sent, post } = posts();
    driver.setActive('markdown');
    expect(sent).toEqual([]);
    driver.ready(post);
  ...
  it('posts nothing while the tour is off, and the active scene when it comes back', () => {
    const driver = createSceneDriver({ tour: false, visible: true });
    const { sent, post } = posts();
    driver.ready(post);
    driver.setActive('versions');
    expect(sent).toEqual([]);
    driver.setTour(true);
    expect(sent).toEqual(['versions']);
    driver.setTour(false);
    driver.setTour(true);
    expect(sent).toEqual(['versions']);
  });
});
...
function setup(options: { wide?: boolean; reduced?: boolean; tokens?: Record<string, string> } = {}) {
  ...
  const stage = element();
  const steps = SCENES.slice(1).map((name) => element({ step: name, anchor: name === 'apps' || name === 'versions' ? 'bottom' : 'top' }));
  ...
  return { tour, attributes, layout, stage, steps, camera, root, observers, resized, doc, env, scroll, fire, flip };
}

describe('startTour', () => {
  it('draws the trigger line at the bottom of the caption band, and redraws it on resize', () => {
    const { observers, env, fire, steps } = setup();
  ...
  it('marks the step at the line and anchors the stage to its edge', () => {
    const { scroll, steps, stage } = setup();
    scroll(0);
    expect(stage.dataset.anchor).toBe('top');
    expect(steps.some((step) => step.classList.contains('is-active'))).toBe(false);
    scroll(900 - LINE + 3 * 800);
    expect(steps[3]?.classList.contains('is-active')).toBe(true);
    expect(steps.filter((step) => step.classList.contains('is-active'))).toHaveLength(1);
  ...
  it('posts scenes through the ready callback as the steps cross the line', () => {
    ...
    const sent: SceneName[] = [];
    scroll(0);
    tour.ready((scene) => sent.push(scene));
    scroll(900 - LINE);
    scroll(900 - LINE + 10);
  ...
  it('stops posting once engaged, and holds while the page is hidden', () => {
    ...
    hidden.tour.ready((scene) => held.push(scene));
    ...
    engaged.tour.ready((scene) => quiet.push(scene));
    engaged.tour.engaged();
  ...
  it('turns the tour attribute off below the breakpoint or under reduced motion, and back on', () => {
    const { attributes, flip, tour, scroll } = setup();
    const sent: SceneName[] = [];
    tour.ready((scene) => sent.push(scene));
  ...
  it('posts nothing as the tour turns off, and the step at the line in the tour layout when it comes back', () => {
    const { tour, flip, layout, steps, observers } = setup();
    const sent: SceneName[] = [];
    tour.ready((scene) => sent.push(scene));
    layout.onTour = (on) => {
      steps.forEach((step, i) => {
    ...
    expect(steps.at(-1)?.classList.contains('is-active')).toBe(true);
    expect(sent).toEqual(['hero']);
    flip('(min-width: 1200px)', true);
